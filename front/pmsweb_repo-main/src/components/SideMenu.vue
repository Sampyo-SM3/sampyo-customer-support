    ...
    <v-list class="tighter-menu-spacing">
      <template v-for="item in processedMenuItems" :key="item.M_CODE">
        ...
        <template v-else-if="item.LEV === 3">
          <v-list-item                     
            v-if="isClickable(item.M_CODE) && (!item.children || item.children.length === 0)"
            @click="activateMenuItem(item)"
            :class="['menu-item', 'no-submenu', { 'active-item': item.isActive }]"
          >
          ...
          >
            ...
            <v-list-item 
              ...
              :key="subItem.M_CODE"
              :title="subItem.M_NAME"
              @click="activateMenuItem(subItem)"
...
:deep(.v-list-item__prepend > .v-icon) {
  margin-inline-end: 8px; /* 아이콘과 텍스트 사이 간격 추가 조정 */  
}
...
.active-item .sub-menu-text {
  color: white; /* 활성 항목 텍스트 색상 (원하는 색상으로 변경 가능) */    
...
:deep(.menu-item.no-submenu.active-item .custom-menu-icon) {
  margin-left: -10px; /* 활성 상태일 때 아이콘을 왼쪽으로 10px 이동 */
}
